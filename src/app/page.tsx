...
export default function LifeGalleryApp() {
  ...
  const handlePhotoAiRequest = () => {
    ...
    setTimeout(() => {
      ...
      // 초기화
      setBrightness(100); setContrast(100); setSaturation(100); setSepia(0); setGrayscale(0); setHueRotate(0); setInvert(0);

      if (p.includes('만화') || p.includes('애니')) {
          setContrast(160); setSaturation(180); setBrightness(110);
          setAiMessage("🎨 사진을 선명한 만화 스타일로 바꿨습니다!");
      } else if (p.includes('회화') || p.includes('그림') || p.includes('유화')) {
          setSepia(30); setSaturation(140); setContrast(120);
          setAiMessage("🖼️ 예술적인 회화 느낌을 더했습니다.");
      } else if (p.includes('흑백') || p.includes('옛날') || p.includes('추억')) {
          setGrayscale(100); setContrast(110);
          setAiMessage("📸 차분한 흑백 사진으로 변환했습니다.");
      } else if (p.includes('화려') || p.includes('반전') || p.includes('꿈')) {
          setHueRotate(180); setInvert(20);
          setAiMessage("✨ 환상적인 분위기로 반전시켰습니다.");
      } else {
          setBrightness(115); setContrast(115); setSaturation(125);
          setAiMessage("🌟 사진을 전반적으로 선명하고 화사하게 보정했습니다.");
      }
    ...
    <div className="animate-fade-in container">
      ...
      {step === 1 && (
        <div className="glass-panel animate-fade-in">
            ...
            {isAiProcessing ? (
              ...
              <img 
               ...
               style={{ 
                 ...
                 filter: `brightness(${brightness}%) contrast(${contrast}%) saturate(${saturation}%) sepia(${sepia}%) grayscale(${grayscale}%) hue-rotate(${hueRotate}deg) invert(${invert}%)`
               }} 
              />
          ...
          {localImage && (
            <>
              {aiMessage && <div className="animate-fade-in" style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#10b981', padding: '10px', borderRadius: '10px', marginBottom: '15px', textAlign: 'center', fontWeight: 'bold' }}>{aiMessage}</div>}
      ...
      {/* STEP 2-5: 로직 유지 (생략 없이 통합 유지) */}
      {step === 2 && (
        <div className="glass-panel animate-fade-in">
          <h1>이야기 정밀 다듬기</h1>
          <div className="montage-item" style={{ marginBottom: '24px', aspectRatio: '4/3', position: 'relative' }}>
            {localImage && <img src={localImage} alt="배경" style={{ position: 'absolute', width: '100%', height: '100%', objectFit: 'cover', filter: `brightness(${brightness}%) contrast(${contrast}%) saturate(${saturation}%) sepia(${sepia}%) grayscale(${grayscale}%)` }} />}
      ...
      {step === 3 && (
        <div className="glass-panel animate-fade-in">
          ...
          <button className="btn btn-primary" onClick={nextStep} style={{ width: '100%', padding: '22px', marginTop: '10px' }}>다음 목소리</button>
